...
REVISION=`echo '$Revision: 1.13 $' | sed -e 's/[^0-9.]//g'`
HPSA="0"
DEBUG="0"
VERBOSE="0"
...
print_usage() {
        ...
        echo " NOTE:"
        echo ""
        echo " HP Array Configuration Utility CLI (hpacucli / hpssacli) need administrator rights."
        echo " Please add this line to /etc/sudoers"
        echo " --------------------------------------------------"
        echo " nagios      ALL=NOPASSWD: /usr/sbin/hpacucli, /usr/sbin/hpssacli"
        echo ""
        echo " NB: Please update the hpacucli to the current version (today 9.40) or >8.70"
}
...
if [ ! -x $hpacucli ]; then
        if [ -x /usr/sbin/hpssacli ]; then
                ...
                echo "ERROR: hpacucli or hpssacli tools should be installed!"
                exit $STATE_UNKNOWN
        fi
fi
